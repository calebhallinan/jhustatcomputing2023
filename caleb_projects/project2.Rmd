...
Exp <- function(x, k) {
    ...
    # make sure x is single number and numeric
    stopifnot(is.numeric(x), length(x) == 1)
    # init return val
    return_val = 0
    # for loop
    for (i in 1:k) {
        return_val = return_val + ((x^i)/factorial(i))
    }
    # return value
    return (1 + return_val)
}
...
sample_sd <- function(x) {
    # make sd
    return (sqrt((1/(length(x) - 1)) * sum((x - sample_mean(x))^2)))
}
...
calculate_CI <- function(x, conf = 0.95) {
    ...
    dof <- length(x) - 1
    # calc t stat
    t_stat <- qt(p = alpha / 2, df = dof, lower.tail = FALSE)
    # get sx
    sdx = (sd(x)/(sqrt(length(x))))
    # get lower bound
    lb = (mean(x) - (t_stat * sdx))
    # get upper bound
    ub = (mean(x) + (t_stat * sdx))
    # make vector
    CI = c(lb, ub)
    # name vector
    names(CI) = c(paste(as.character(((alpha*100) / 2)), "%"), paste(as.character(((alpha*100) / 2) + (conf*100)), "%"))
    # return
    return (CI)
}
...
calculate_CI(x, conf = .95)
calculate_CI(x, conf = .90)
...
# 3. convert city name to all upper
rainfall =  rainfall %>% 
    mutate(across(city_name, str_to_upper))

# 4. join dataframes
df = inner_join(rainfall, temperature, by = c("city_name","date"))

# NOTE: why do I have 15 columns instead?
dim(df)
```
...
# create ggplot object with only data 2014 and onwards
ggplot(filter(df, year >= 2014), aes(x = date)) +
    # filter data for just max temperature
    geom_line(data = filter(df, year >= 2014 & temp_type == "max"), aes(y = temperature, color = temp_type), linewidth = .5) + 
    # filter data for just min temperature
    geom_line(data = filter(df, year >= 2014 & temp_type == "min"), aes(y = temperature, color = temp_type), linewidth = .5) +
    # change the color to red and blue, change labels
    scale_color_manual(values = c("red", "blue"), labels = c(expression("Maximum"), expression("Minimum"))) +
    # facet by city
    ...
    labs(
        x = "Time (Daily)",
        y = "Temperature (\u00B0C)",
        title = "Temperature Minimum and Maximum of Australian Cities Over Time",
        # found a cool solution to subtitle issues I had last time!!
        subtitle = str_wrap("Overall, it seems that all cities follow a similar trend in temperature. However, cities like Canberra and Sydney have more variation day-to-day as well as season-to-season in their temperature compared to the other cities. Melbourne may have the least variation in temperature type, although sometimes it has random hot days.", width = 90),
        caption = "Created By: Caleb Hallinan",
        color = "Temperature Type"
        ) +
    # get best theme
    theme_bw() + 
    # change legend position
    theme(legend.position = c(.95, 0.1), legend.justification = c(1, 0), plot.title = element_text(face = "bold"))
...
```{r}
# create a function 
EDA_plots <- function(city_name, year) {
...
# 1. filter data for 2014 and on
rain_df = filter(df, year >= 2014)

# 2. calc mean, sd, CI
rain_df = rain_df %>% 
    # group by respective variables
    group_by(city_name, year) %>% 
    # summarize everything asked for
    summarize(sample_mean = sample_mean(rainfall),
              # my sd func
              sample_sd = sample_sd(rainfall),
              # get just lower bound of sd func
              lower_bound = calculate_CI(rainfall, conf = .95)[1],
              # get just upper bound of sd func
              upper_bound = calculate_CI(rainfall, conf = .95)[2])
...
# init plat
ggplot(rain_df, aes(x = year, color = city_name)) +
    # make points for each
    geom_point(data = rain_df, aes(y = sample_mean)) +
    # make lines for each
    geom_line(data = rain_df, aes(y = sample_mean)) +
    # make error bar for each
    geom_errorbar(aes(ymin = lower_bound, ymax = upper_bound)) +
    # facet wrap by city
    facet_wrap(. ~ city_name) +
  # change colors
    scale_colour_manual(values = paletteer_d("ggprism::colors", 5)) +
    # make labels for everything
    labs(
        x = "Time (Years)",
        y = "Rainfall (mm)",
        title = "Average Rainfall Over Time of Australian Cities",
        # found a cool solution to subtitle issues I had last time!!
